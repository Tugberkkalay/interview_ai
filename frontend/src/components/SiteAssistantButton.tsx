...
- **Kapanış mesajı örneği:** "Çok memnun oldum, size yardımcı olabildiysem ne mutlu bana. COGNIVIA hakkında başka sorularınız olursa her zaman buradayız. İyi günler dilerim!"`;

                // Demo request tool definition
                const createDemoRequestTool: FunctionDeclaration = {
                    name: "create_demo_request",
                    description: "Kullanıcı demo istediğinde veya ilgi gösterdiğinde BU FONKSİYONU HEMEN ÇAĞIR. Hiçbir parametre göndermene gerek yok, hepsini boş bırakabilirsin. Bu fonksiyon ekranda bir form (modal) açar. Kullanıcı bilgileri oraya kendisi girecek. Sen sözlü olarak bilgi toplamaya ÇALIŞMA.",
                    parameters: {
                        type: Type.OBJECT,
                        properties: {
                            name: { type: Type.STRING, description: "Kullanıcının adı (varsa)" },
                            company: { type: Type.STRING, description: "Kullanıcının şirketi (varsa)" },
                            preferredTime: { type: Type.STRING, description: "Tercih edilen tarih (varsa)" },
                            contact: { type: Type.STRING, description: "Contact (varsa)" }
                        },
                        required: []
                    }
                };
                        ...
                        onopen: async () => {
                                ...
                                setTimeout(() => {
                                    if (isSessionActiveRef.current && session) {
                                        ...
                                        session.sendRealtimeInput({
                                            text: "Lütfen şu cümleyi tam olarak seslendir: 'Merhaba, COGNIVIA Mülakat Uygulaması'na hoşgeldiniz. Ben COGNIVIA Asistanıyım, nasılsınız bugün?'"
                                        });
                                    }
                                }, 500);
                            } catch (err) {
                            ...
                            processor.onaudioprocess = (e) => {
                                ...
                                if (rms > speechThreshold) {
                                    ...
                                    const silenceTimeout = 1500; // Sessizlik süresini 1.5 saniyeye çıkardık
                                    ...
                                    if (timeSinceLastSpeech <= silenceTimeout) {
                                        ...
                                        if (isUserSpeakingRef.current) {
                                            const speechDuration = now - (speechStartTimeRef.current || 0);
                                            if (speechDuration < 500) {
                                                console.log("Ignored short speech/noise:", speechDuration, "ms");
                                                isUserSpeakingRef.current = false;
                                                return;
                                            }
                                            isUserSpeakingRef.current = false;
                                ...
                                for (const fc of msg.toolCall.functionCalls) {
                                    if (fc.name === 'create_demo_request') {
                                        try {
                                            ...
                                            // Validation KALDIRILDI - Artık boş argümanlarla da açılabilir
                                            // Form UI tarafında zorunlu alanları kontrol edecek

                                            // State'i güncelle (varsa gelen bilgileri koy, yoksa boş)
                                            ...
                                            sessionPromise.then(session => {
                                                session.sendToolResponse({
                                                    functionResponses: [{
                                                        id: fc.id,
                                                        name: fc.name,
                                                        // Başarılı yanıt gönder, AI konuşmaya devam edebilir
    ...
    const getButtonText = () => {
        ...
        return "COGNIVIA Mülakat Asistanı ile konuş";
    };
